...
- **Milestone 5 — フロントエンド全文検索（facet 絞り込み・ハイライト・ページング）**:
  - **全文検索 UI**（`frontend/lib`＋`frontend/components`）: `SearchFilters`/`Facet`/`SearchResultPage` 型（`lib/types/search.ts`）、`buildSearchParams`（`filter[fulltext]`・`filter[channel]`・`filter[slack_user]`・`filter[posted_at][value]+[operator]=>=`・`page[limit/offset]`・`sort=-posted_at`、`SEARCH_PAGE_SIZE=20`）、`fetchSearchResults`（`buildUrl("/jsonapi/index/slack_messages", …)`＋`fetch`。`getResourceCollection` は index ルート非対応のため不使用。`hasNext` は `links.next` 有無を優先し `data.length===20` でフォールバック）。
  - **facets 消費**: `mapFacets`（`meta.facets` 欠落耐性。`empty_behavior:none` の facet 省略に対応）、`toggleFacetValue`（同値クリックで解除・`offset` を 0 リセット）。`FacetSidebar` は state 起点の「適用中フィルタ」chip で、0 件で省略された facet も解除可能。
  - **UI**（Mantine ＋ TanStack Query）: `SearchBar`（submit 型・Enter/ボタン確定・× でクリア）、`FacetSidebar`（channel/slack_user/posted_at facet 一覧＋件数・active 強調）、`SearchResultItem`/`SearchResultList`（Mantine `Highlight` によるクライアント側ハイライト＝`<mark>`・`dangerouslySetInnerHTML` 不使用、loading/error/0 件出し分け、前へ/次へページング、`totalCount` 非 null 時のみ「全 N 件」）、`useSearch`（`enabled: fulltext 非空`・`placeholderData: keepPreviousData`）。`BrowsePanel` に検索モード切替（検索中は Navbar=`FacetSidebar`・Main=`SearchResultList`、クリアで閲覧モード復帰・選択チャンネル保持）。
  - **チャンネル閲覧の追加読み込み**（M4 申し送り解消）: `buildChannelMessagesParams(tid, offset)` に `page[offset]`、`fetchChannelMessages` が `{ messages, hasNext }` を返す（`links.next` 優先・`length===100` フォールバック、`CHANNEL_MESSAGES_PAGE_LIMIT=100`）、`useChannelMessages` を `useInfiniteQuery` 化、`MessageList` に「さらに読み込む」ボタン。
  - 注: バックエンドはハイライトを返さない（highlight プロセッサ未使用）ためクライアント側実装。CJK 2 文字クエリは `search_api_db` bigram の既知制約で非対応（`docs/spec/jsonapi-search.md §9.1`、変更なし）。新規 ADR なし（ADR-0005 / ADR-0007 の範囲内）。
  - テスト: small（Vitest）＝検索クエリビルダ/`fetchSearchResults`・facet mapper/toggle・`SearchBar`・`FacetSidebar`・`useSearch`・`SearchResultList`・`BrowsePanel` 検索モード・ページング（slack-api/hook/`MessageList`）。large（Playwright）＝`search.spec.ts` 新規・`browse.spec.ts` に load-more シナリオ追加。
- `docs/spec/` 追加: `frontend-search.md`（M5 データフロー仕様）。`frontend-browse.md` の §2/§7/§9 をページング実装済みに更新。
